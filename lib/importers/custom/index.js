...
async function importData(source) {
  const {importer, config, functions} = source

  const resources = await bluebird.props(mapValues(config.resources, async (resourceConfig, resourceName) => {
    console.time(`${importer} - fetching ${resourceName}`)
    const resource = await fetchResource(resourceConfig.url)
    console.timeEnd(`${importer} - fetching ${resourceName}`)
    console.time(`${importer} - processing ${resourceName}`)
    ...
    console.timeEnd(`${importer} - processing ${resourceName}`)
    return result
  }))

  let errored = 0

  console.time(`${importer} - consolidating`)
  ...
  console.timeEnd(`${importer} - consolidating`)
