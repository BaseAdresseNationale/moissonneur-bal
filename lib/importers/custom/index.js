...
const {importData: importBALData} = require('../bal')
...
const convertersConfig = require('./config')
...
async function importData(source) {
  const {config, functions} = convertersConfig[source.converter]
  ...
  return importBALData({resources: {default: {data: convertedFile}}})
}
