...
async function writeFile(buffer, filename, metadata = {}) {
  ...
  try {
    ...
    // SAVE TO MONGO (DELETE AT END OF MIGRATION)
    await mongo.writeFile(_id, buffer, filename, metadata)
...
async function getFile(fileId) {
  ...
  try {
    ...
    console.log(`<<FILE>> Fichier non trouvé sur S3 ${fileId}`)
    return mongo.getFile(_id)
  }
}
