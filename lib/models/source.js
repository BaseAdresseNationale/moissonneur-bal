...
async function askHarvest(sourceId) {
  const {modifiedCount} = await mongo.db.collection('sources').updateOne(
    {_id: sourceId, 'harvesting.harvestingSince': null, harvestAsked: {$ne: true}},
    {$set: {harvestAsked: true}}
  )
...
async function setHarvestAskedToFalse(sourceId) {
  await mongo.db.collection('sources').updateOne({_id: sourceId}, {$set: {harvestAsked: false}})
}
