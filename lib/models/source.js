...
async function finishHarvesting(sourceId, changes) {
  ...
  if (changes.updateStatus === 'updated') {
    ...
    changesForSource['data.dataHash'] = changes.dataHash
...
async function writeFile(sourceId, file, hash, dataHash) {
  const timestamp = (new Date()).toISOString().replace(/[-:.]/g, '')
  const filename = `${sourceId}-${timestamp}.csv`
  return mongo.writeFileBuffer(file, filename, {sourceId, hash, dataHash})
}
