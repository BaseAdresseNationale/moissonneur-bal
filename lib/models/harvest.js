...
async function getHarvest(harvestId) {
  return mongo.db.collection('harvests').findOne({_id: mongo.parseObjectID(harvestId)})
}
...
async function getLatestHarvests(sourceId) {
  return mongo.db.collection('harvests').find({sourceId}, {sort: {finishedAt: -1}}).limit(10).toArray()
}
