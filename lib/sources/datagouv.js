...
async function getEligibleBALDatasets() {
  const response = await got('https://www.data.gouv.fr/api/1/datasets/?tag=base-adresse-locale&page_size=100', {responseType: 'json'})
  ...
  const datasets = await Promise.all(
    response.body.data
      .filter(d => d.resources.some(r => isBAL(r)) && d.organization)
      .map(async d => {
