...
async function handleNewFile({sourceId, harvestId, newFile, newFileHash, currentFileId, currentFileHash, currentDataHash, organizationName}) {
  if (!sourceId || !newFile) {
  ...
  const result = await validate(newFile, {profile: '1.3-etalab', relaxFieldsDetection: true})
  ...
  const changes = await handleCommunesData({sourceId, harvestId, rows: result.rows, organizationName})
