...
export class UpdateSourceOrganisationWorker implements Worker {
  ...
  async upsertOrgnizations(datasets: DatasetDataGouv[]) {
    ...
    this.organizationService.softDeleteInactive(
      organizations.map(({ _id }) => _id),
    );
  }

  async upsertSources(datasets: DatasetDataGouv[]) {
    ...
    this.sourceService.softDeleteInactive(sources.map(({ _id }) => _id));
  }
